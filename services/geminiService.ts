...
export const generateWorldData = async (): Promise<WorldData | null> => {
    try {
        ...
        if (!startId || !locations.some((l: MapLocation) => l.id === startId)) {
            console.warn("startLocationId from AI is invalid or missing. Using the first location as a fallback.");
            if (locations.length > 0) {
                startId = locations[0].id;
            } else {
                console.error("No locations were generated by the model. Cannot create world.");
                return null;
            }
        }
